<!DOCTYPE html>
<html lang="en">
<!-- insert page head -->
<?php include "layout/head.php"; ?>

<body>
    <!-- Top Bar Start -->
    <?php include "layout/topbar.php"; ?>
    <!-- Top Bar End -->

    <!-- Brand Start -->
    <?php include "layout/brand.php"; ?>
    <!-- Brand End -->

    <!-- Nav Bar Start -->
    <?php include "layout/navbar.php"; ?>
    <!-- Nav Bar End -->

    <div class="main-news py-5">
        <div class="container">
            <div class="row">
                <div class="col-lg-9">
                    <div class="row">
                        <!-- Category Name Heading -->
                        <?php
                        if (isset($_GET['category']) && !empty($_GET['category'])) {
                            $categoryId = $_GET['category'];

                            $sql = "SELECT * FROM categories WHERE id=:categoryId";
                            $stmt = $conn->prepare($sql);
                            $stmt->bindParam(':categoryId', $categoryId, PDO::PARAM_INT);
                            $stmt->execute();
                            $category = $stmt->fetch(PDO::FETCH_OBJ);
                        ?>
                            <div class="col-12">
                                <h1><?= $category->name; ?></h1>
                            </div>

                            <!-- Category-specific News -->
                            <?php
                            $sql = "SELECT * FROM posts WHERE category_id = :categoryId";
                            $stmt = $conn->prepare($sql);
                            $stmt->bindParam(':categoryId', $category->id, PDO::PARAM_INT);
                            $stmt->execute();
                            $categoryPosts = $stmt->fetchAll(PDO::FETCH_OBJ);

                            if ($categoryPosts != null) {
                                foreach ($categoryPosts as $categoryPost) { ?>
                                    <div class="col-md-4">
                                        <div class="mn-img">
                                            <img src="admin/<?= $categoryPost->image; ?>" />
                                            <div class="mn-title">
                                                <a href="view.php?id=<?= base64_encode($categoryPost->id); ?>"><?= $categoryPost->title; ?></a>
                                            </div>
                                        </div>
                                    </div>
                        <?php }
                            } else {
                                echo "No news available for the requested category.";
                            }
                        }
                        ?>
                    </div>
                </div>
                
                <!-- Sidebar -->
                <div class="col-lg-3">
                    <?php include "layout/sidebar.php"; ?>
                </div>
            </div>
        </div>
    </div>

    <!-- Footer Start -->
    <?php include "layout/footer.php"; ?>
    <!-- Footer End -->

    <!-- Back to Top -->
    <a href="#" class="back-to-top"><i class="fa fa-chevron-up"></i></a>

    <!-- JavaScript Libraries -->
    <script src="https://code.jquery.com/jquery-3.4.1.min.js"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.bundle.min.js"></script>
    <script src="lib/easing/easing.min.js"></script>
    <script src="lib/slick/slick.min.js"></script>

    <!-- Template Javascript -->
    <script src="js/main.js"></script>
</body>

</html>